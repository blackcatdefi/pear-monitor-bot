...
Triggers actuales (cada uno con input LIVE — nunca valores fabricados):
  ...
  2. PM aave-HF < 1.10 (zona crítica → suggest_close) — fuente ÚNICA
  ...
  3. Basket UPnL < -$2,000 (drawdown extremo → alert_only) — live perps.
...
R-SIGNAL-DIET (2026-07-20): eliminados DOS triggers más:
  * ``btc_dca_63_65`` — era una alerta DCA de precio disfrazada de kill
    trigger; disparaba nightly y ahogaba las alertas reales. BCD ve esos
    niveles en sus charts.
  * ``btc_near_dreamcash_liq`` — la wallet DreamCash 0x171b está VACÍA
    (accountValue=0, verificado on-chain 2026-07-20); el trigger solo podía
    reportar n/d para siempre. Peso muerto eliminado.
El registro queda en EXACTAMENTE 3 triggers. pm_hf_below_110 es CRÍTICO y
permanece intacto.
...
# R-SIGNAL-DIET (2026-07-20): ``_evaluate_btc_dca_zone`` (btc_dca_63_65) y
# ``_evaluate_btc_near_dreamcash_liq`` (+ dreamcash_liq_proximity) ELIMINADOS
# del registro. El primero era una DCA price alert disfrazada; el segundo
# apuntaba a una wallet vacía (0x171b accountValue=0). Registro = 3 triggers.
