...
async def compute_kill_scenarios() -> str:
    ...
    # 2. DreamCash — wallet de rescate / hedge (margen separado del PM core)
    lines.append("2\ufe0f\u20e3 DREAMCASH \u2014 RESCATE/HEDGE (0x171b, margen separado)")
    lines.append("   Estado: ACTIVA \u2014 short baskets (OP, XPL, ETHFI/GRASS) + shield USDC")
    lines.append("   Kill scenario: risk-on squeeze sostenido comprime los shorts")
    lines.append("   Triggers concretos:")
    lines.append("   \u2022 Ceasefire confirmado + Fed dovish + alts +30% (squeeze)")
    lines.append("   Rol PM: hedge INDIRECTO + shield USDC para defensa MANUAL del PM core")
    lines.append("   (NO protege al PM in-wallet; el liq price/aave-HF del PM es el riesgo real)")
    lines.append("")
