...
def format_quick_positions(
    ...
    # ── Trade del Ciclo (BTC LONG Blofin — manual tracking) ──
    if cycle_trade_block:
        lines.append("")
        lines.append(cycle_trade_block)

    # ── Deltas vs previous snapshot ──
    if delta_block:
        lines.append(delta_block)  # already has leading blank line
